...
**正确的发现方式**(本仓库已提供 3 种,任选一种):

**方式 A: 读发现文件**(零依赖)
`start_server.sh` 启动时把 `--model` 参数原样写到 `/tmp/qwen-local-model-id`,
任何客户端读这个文件就拿到正确 id:

```python
# Python
model_id = open("/tmp/qwen-local-model-id").read().strip()

# Bash
MODEL_ID=$(cat /tmp/qwen-local-model-id)
```

**方式 B: 走 bot 的 `/api/model` 端点**(HTTP)
bot 启动时把同一 id 暴露到 :8090:

```bash
curl -s http://127.0.0.1:8090/api/model
# {"model": "/Users/.../Qwen3.6-...", "discovery_file": "/tmp/...", "upstream": "http://127.0.0.1:8080/v1"}
```

**方式 C: 直接写死绝对路径**(最简,适合 read-only config)
直接抄绝对路径,接受"模型路径变了客户端要改"。

> 切到 llama.cpp / vLLM 后端时这个问题会消失(它们允许 model 字段为任意字符串
> 或自动匹配)。所以这些"自动发现"机制是为了让客户端代码在 mlx-vlm 上
> 跟其他后端一样"用什么 id 都行",未来切后端时减少改动面。
...
- [ ] mlx-vlm 后端:**不要**用 `/v1/models` 的 id;改读发现文件
      `/tmp/qwen-local-model-id` 或 GET `http://127.0.0.1:8090/api/model`。
      其他后端(llama.cpp / vLLM)无此限制
